...
std::vector<int> inferOnImage(const std::string &path, const std::string &preprocess, Ort::Session &session,
  ...
  size_t input_tensor_size = 1428 * 2128 * 4;  // simplify ... using known dim values to calculate size
                                             // use OrtGetTensorShapeElementCount() to get official size!
  //input tensor is 1 x 1428 x 2128 x 4
  ...
  dimY = 2128;
  numChannels = 4;
  input_tensor_size = dimX * dimY * numChannels;
  ...
  fs.read((char *) input_tensor_values.data(), sizeof(float) * input_tensor_size);
  fs.close();
  fs.clear();
  /*if (arr == nullptr) {
  ...
  /*if (arr.shape.size() != 4 || dimX != 1428 || dimY != 2128 || numChannels != 4) {
    printf("Input tensor has the wrong size \nNumber of dimensions:%d\nDimensions:%d,%d,%d\n", arr.shape.size(), dimX, dimY, numChannels);
    exit(-1);
  }*/
  ...
  //std::vector<float>* input_tensor_values = arr.data<std::vector<float>>();
  ...
  */
  ...
  //cnpy::npy_save("output.npy", floatarr, {1, 2484, 4256, 3}, "w");
  //ofstream ofs;
  //ofs.open("output.data", ios::out | ios::app | ios::binary);
  FILE *f = fopen("output.data", "wb");
  if (f == nullptr) {
    printf("Failed to open output file\nError %d\n", errno);
  }
  fwrite(floatarr, sizeof(float), output_tensors[0].GetTensorTypeAndShapeInfo().GetElementCount(), f);
  fclose(f);
...
int main(int argc, char* argv[]) {
  ...
  if (mlockall(MCL_CURRENT | MCL_FUTURE) != 0) {
    ...
    printf("Get read for more print statements (hopefully)\n");
  }
#endif
  ...
  printf("entering trace condition\n");
  //trace option commented out!!
  printf("commented out trace option");
  if (cmd.count("trace")) {
    printf("entered trace option\n");
    const char* option = cmd["trace"].as<std::string>().c_str();
    session_options.EnableProfiling(option);
  }
